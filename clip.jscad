function getParameterDefinitions(){
    return [
        {name:'r',  caption:'radius:',    type:'float',initial:58,min:0.1,max:4200,step:0.1},
...
function main (p) {
	...
	var path=new CSG.Path2D([[circle.points[0]._x+p.r/2/3,circle.points[0]._y+p.r/2/3/2]],false);
	path=path.concat(circle);
    path=path.appendPoint([path.points[path.points.length-1]._x+p.r/2/3,path.points[path.points.length-1]._y-p.r/2/3/2]);
